...
public class AssignmentTask2{
    ...
    public static Node organizeBooks(Node head, Integer[] popularity) {
       //as we have to sort linkedlist so bubble sort should work
        ...
        while(i<n-1 &&temp.next!=null){
            if(popularity[i]<popularity[i+1]){
                //swapping array
                Integer store= popularity[i];
                popularity[i]=popularity[i+1];
                popularity[i+1]=store;
                //swapping elements in node not the nodes
                String temp_data = (String)temp.elem;
                temp.elem = temp.next.elem;
                temp.next.elem = temp_data;
